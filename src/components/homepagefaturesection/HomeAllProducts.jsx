    ...
    const handleFetchProduct = async (isFetchMore) => {
        ...
        try {
            if (latitude && longitude) {
                const response = await api.getProductByFilter({ latitude: latitude, longitude: longitude, filters: { limit: totalProductsPerPage, offset: localOffset } });
                if (response.status == 1) {
                    setTotalProducts(response.total)
                    isFetchMore ? setAllProducts((products) => [...products, ...response.data]) : setAllProducts(response.data)
                    setLoading(false)
