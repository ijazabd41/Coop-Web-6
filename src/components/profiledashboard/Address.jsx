    ...
    <div className="w-full cardBorder rounded-sm ">
      <div className="backgroundColor flex justify-between p-4 items-center">
        ...
        {addresses?.allAddresses?.length > 0 && (
          <button
            className=" flex items-center gap-2 py-1 px-1.5 md:py-2 md:px-3 rounded-sm text-base font-medium primaryBackColor text-white"
      ...
      <div className="">
        {loading ? (
          Array?.from({ length: 6 })?.map((_, index) => {
            return <CardSkeleton key={index} height={200} padding="2px" />;
          })
        ) : addresses?.allAddresses?.length > 0 ? (
