...
const ActiveOrdersCard = ({ order }) => {
  ...
  const getOrderStatus = () => {
    switch (order?.active_status) {
      case "1":
        return (
          <span className="p-2 text-center w-10/12 paymentPendingStatus rounded-sm text-base font-bold ">
            {t("paymentPending")}
          </span>
        );
      case "2":
        return (
          <span className="p-2 text-center w-10/12 orderRecieved rounded-sm text-base font-bold">
            {t("order_status_display_name_recieved")}
          </span>
        );
      case "3":
        return (
          <span className="p-2 text-center w-10/12 orderProcessed rounded-sm text-base font-bold">
            {t("processed")}
          </span>
        );
      case "4":
        return (
          <span className="p-2 text-center w-10/12 rounded-sm orderShipped text-base font-bold">
            {t("order_status_display_name_shipped")}
          </span>
        );
      case "5":
        return (
          <span className="p-2 text-center w-10/12 orderOutForDelivery rounded-sm text-base font-bold">
            {t("out_for_delivery")}
          </span>
        );
      case "6":
        return (
          <span className="p-2 text-center w-10/12 orderDelivered rounded-sm text-base font-bold">
            {t("order_status_display_name_delivered")}
          </span>
        );
      case "7":
        return (
          <span className="p-2 text-center w-10/12 rounded-sm orderCancelled text-base font-bold">
            {t("cancelled")}
          </span>
        );
      case "8":
        return (
          <span className="p-2 text-center w-10/12 orderReturned rounded-sm text-base font-bold">
            {t("returned")}
          </span>
        );
      case "9":
        return (
          <span className="p-2 text-center w-10/12 orderInProcess rounded-sm text-base font-bold">
            {t("order_in_process")}
          </span>
        );
      case "10":
        return (
          <span className="p-2 text-center w-10/12 orderReadyToPickup rounded-sm text-base font-bold">
            {t("ready_to_pickup")}
          </span>
        );
      case "9":
        return (
          <span className="p-2 text-center w-10/12 orderPicked rounded-sm text-base font-bold">
            {t("picked")}
          </span>
        );
      default:
      ...
      <div className="py-3 px-4">
        <div className="w-full cardBorder rounded-md">
          <div className="flex flex-col gap-3 lg:gap-0 md:grid grid-cols-12 p-4 border-b-2">
            ...
            <div className="col-span-4 lg:col-span-3 flex flex-col items-start md:items-end">
              <p className="font-normal text-sm SecondaryTextColor">
                {t("orderStatus")}
              </p>
              {getOrderStatus()}
